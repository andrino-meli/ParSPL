...
double FanInVal[N_CCS] __attribute__((section(".l1"), aligned(8)));
// #### parallel implementation of QDLDL_Lsolve ####
// setup indirection for all
// setup 1 to indirect stream from bp[Li[len]]
// setup 0 to indirect stream to bp[Li[len]]
// setup 2 to stream from Lx[len]
//void SSSR_PQDLDL_Lsolve(uint32_t core_id){
//    asm volatile("_sssr_psolve_csc_FE: \n":::);
...
//        }
//    } else { // computation
//        asm volatile("_sssr_psolve_csc_FE2: \n":::);
//        __RT_SSSR_BLOCK_BEGIN
...
//           [stride]"r"(8), [lxbase]"r"(&LxNew[base]), [libase] "r"(&LiNew[base]) : "memory"
//        );
//        double* valptr = b;
//        uint8_t* lenarr = &LpLen[LINSYS_N*core_id];
//        uint8_t len = *lenarr;
//        if(LINSYS_N % 2 == 1) {
//            if (len != 0) {
//                asm volatile(
...
//                __RT_SSSR_SCFGWI(%[len], 31, __RT_SSSR_REG_BOUND_0)
//                __RT_SSSR_SCFGWI(%[libase], 0, __RT_SSSR_REG_RPTR_INDIR) // change stream wr->rd rd->wr
...
//            }
//        }
//        for( ; lenarr < &LpLen[LINSYS_N*core_id + LINSYS_N ]; ){
//            if (len != 0) {
//                asm volatile(
//                "addi %[len], %[len], -1 \n"
//                // shadow configure SSSR's
...
//        __RT_SSSR_BLOCK_END
//    }
//    __rt_barrier(); //make sure last computation is synchronized.
//}
//
//
//// #### parallel implementation of QDLDL_Lsolve ####
...
//        for(int i = LINSYS_N-1; i>=0; i--){
//            __rt_barrier(); // also emit barrier loads
//            __rt_barrier();
//        }
//        __rt_barrier(); //synchronize last write to bp[i]
//    } else {
...
//        [icfg]"r"(__RT_SSSR_IDXSIZE_U16), [stride]"r"(-8),
//        [lxbase]"r"(&LxNew[top])
//        : "memory"
//        );
//        uint8_t* lentop = &LpLen[LINSYS_N*core_id + LINSYS_N - 1];
...
//        );
//        lentop--;
//        double* valptr = &b[LINSYS_N-1];
//#pragma nounroll
//        for(; valptr >= b;){
//            register double val = 0;
//            if (len != -1) {
//                asm volatile(
//                "csrr    zero,0x7c2                  \n" // synchronize before loading bp[i]s
//                __RT_SSSR_SCFGWI(%[libase], 1, __RT_SSSR_REG_RPTR_INDIR)
//                "frep.o   %[len], 1, 1, 0            \n"
...
//}
void SSSR_PQDLDL_Lsolve(uint32_t core_id){
  if(core_id == CORE_NUM - 1){ // DMA management -> last core manages DMA
    #pragma nounroll
    for(unsigned int i = 0; i < N_SYNCHRO; i++) {
      __rt_barrier();
    }
  }
  else { // computation
    ...
    asm volatile( // SSSR constant config
        ...
        __RT_SSSR_SCFGWI(%[stride], 2, __RT_SSSR_REG_STRIDE_0)
        __RT_SSSR_SCFGWI(%[lxbase], 2, __RT_SSSR_REG_RPTR_0)
        :: [streamlen]"r"(LpLenSum[core_id]-1), [bp]"r"(bp), [icfg]"r"(__RT_SSSR_IDXSIZE_U16), [stride]"r"(8), [lxbase]"r"(&LxNew[base]), [libase] "r"(&LiNew[base]) : "memory"
    );
    float_x* valptr = bp;
    uint8_t* lenarr = &LpLen[N_SYNCHRO*core_id];
    uint8_t len = *lenarr;
    if(N_SYNCHRO % 2 == 1) {
      if (len != 0) {
        asm volatile(
    ...
    for( ; lenarr < &LpLen[N_SYNCHRO*core_id + N_SYNCHRO ]; ){
      if (len != 0) {
        asm volatile(
            // duplicate len
            //"mv t6, %[len]                  \n"
            ...
            // shadow configure SSSR's
            __RT_SSSR_SCFGWI(%[len], 31, __RT_SSSR_REG_BOUND_0)
            //__RT_SSSR_SCFGWI(%[libase], 0, __RT_SSSR_REG_WPTR_INDIR) // TODO: check legality of this.
            __RT_SSSR_SCFGWI(%[libase], 1, __RT_SSSR_REG_RPTR_INDIR)
            __RT_SSSR_SCFGWI(%[libase], 0, __RT_SSSR_REG_WPTR_INDIR)
            "fmv.x.w a6, fa0                 \n" //_rt_fpu_fence_full();
            "mv zero, a6                     \n" //_rt_fpu_fence_full();
