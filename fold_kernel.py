...
# TODO: This is far from production ready !!!
...
def func_verify_Lsolve(L, codedata, dtype=np.float64, transpose=False):
    ...
    if fun[fi] > 1e-3:
        eprint(
            escape.RED,
            f"Golden max err. {fun[fi]} after sol-back-mulp at index {fi}",
            escape.END,
        )
    else:
    ...
    if gun[gi] > 1e-3:
        eprint(
            escape.RED,
            f"SolveX max err. {gun[gi]} after sol-back-mulp at index {gi}",
            escape.END,
        )
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--test", "-p", nargs="?", default="_HPC_3x3_H2", help="Problem to be used."
    )
    parser.add_argument(
        "--fsolve", "-f", action="store_true", help="Verification of Fold solve."
    )
    parser.add_argument(
        "--ldl", "-l", action="store_true", help="Verification of LDLsolve"
    )
    parser.add_argument("--lsolve", action="store_true", help="Verification of Lsolve")
    parser.add_argument(
        "--ltsolve", action="store_true", help="Verification of Ltsolve"
    )
    parser.add_argument("--debug", action="store_true", help="Use debug Matrix")
    parser.add_argument(
        "--vverbose", "-vv", action="store_true", help="Very Verbose logging."
    )
